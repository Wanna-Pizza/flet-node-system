...
class SpatialHashGrid {
  ...
  ({int x, int y}) _getGridIndex(Offset point) {
    ...
    if (!dx.isFinite || !dy.isFinite || !cellSize.isFinite || cellSize <= 0) {
      return (x: 0, y: 0);
    }

    final double fx = dx / cellSize;
    final double fy = dy / cellSize;

    final int xi = fx.isFinite ? fx.floor() : 0;
    final int yi = fy.isFinite ? fy.floor() : 0;
  ...
  Set<({int x, int y})> _getCoveredCells(Rect rect) {
    ...
    // If indices are inverted or invalid, return empty set to avoid large loops.
    if (bottomRight.x < topLeft.x || bottomRight.y < topLeft.y) {
      return {};
    }

    // Protect against unrealistically large ranges that could hang the UI.
    const int maxSpan = 1000;
    final int spanX = bottomRight.x - topLeft.x;
    final int spanY = bottomRight.y - topLeft.y;
